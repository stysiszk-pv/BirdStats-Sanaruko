...
# データの読み込み
df_display, df_tax, df_weath, df = data_loader.load_bird_data()
...
with tab1: 
    if selected_species:
        # 年を文字列として設定し、順序を保持
        plot_data = df_max.copy()
        plot_data['year_label'] = plot_data['biological_year'].apply(lambda x: f'{int(x)}')
        plot_data.set_index('year_label', inplace=True)
        st.line_chart(plot_data[selected_species])
    else: 
...
with tab4: 
    if selected_species: 
        # 年月を文字列として設定し、順序を保持
        plot_data = df_blyr.copy()
        plot_data['date_label'] = plot_data['date'].dt.strftime('%Y-%m')
        st.line_chart(plot_data.set_index('date_label')[selected_species])
    else: 
        st.info('表示する種を選択してください.')
st.write('#')
