...
with st.expander('概要', expanded=True):
    ...
    "40年近くにわたる詳細な鳥類群集データが更に広く共有され受け継がれることを願い, 有志会員の手により, 観測記録のデジタル化が進められています (1998年まで完了, 2025/08/16現在)." \
    ...
    st.markdown('''                
                ...
                左のタブで種ごとの個体数の年次推移を確認できます.   
                右のタブでは各年の最大個体数を観測した月を確認できます.
                ''')
...
tab1, tab2, tab3 = st.tabs(["種ごとの年次推移", "最大個体数を観測した月", "種ごとの月別個体数"])
...
with tab2: 
    if selected_species: 
        st.dataframe(df_summary[selected_species])
    else: 
        st.info('表示する種を選択してください.')
...
st.text('Last updated: 2025/08/16')